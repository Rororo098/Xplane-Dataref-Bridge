"""
Complete Arduino-to-PC Message Examples for X-Plane Dataref Bridge

This file provides working examples for both Arduino and PC sides of the communication protocol.
"""

# ARDUINO SIDE EXAMPLE (to be used in .ino file)
"""
// Arduino Message Functions Example
// This code should be placed in your Arduino .ino file
...
// Example usage in Arduino loop
void loop() {
  // Example: Send button press notification
  sendInputNotification("BUTTON_1", "PRESSED");
  
  // Example: Send command execution
  sendCommand("SET_MODE_AUTO");
  
  // Example: Send dataref write request
  sendDatarefWrite("sim/cockpit/electrical/avionics_on", "1");
  
  // Example: Send acknowledgment
  sendAck("INIT", "COMPLETE");
  
  // Example: Send value response
  sendValueResponse("sim/flightmodel/position/altitude", "1500.5");
  
  // Example: Send array value
  sendArrayValue("sensor_array", "float", "1.0,2.5,3.7,4.2,5.1");
  
  // Example: Send element value
  sendElementValue("servo_positions", 2, "int", "1500");
  
  delay(2000); // Wait 2 seconds before sending next batch
}
"""
...
class ArduinoMessageHandler:
    ...
    def handle_input(self, msg: Dict[str, Any]) -> bool:
        ...
        # Trigger specific actions based on the input
    ...
    def handle_arrayvalue(self, msg: Dict[str, Any]) -> bool:
        ...
        values_str = msg['values']
        
        print(f"Array value reported: {array_name} ({data_type}) = {values_str}")
        
        # Convert CSV string to list
        values_list = [val.strip() for val in values_str.split(',')]
        
        # Convert values based on type
        converted_values = self.convert_values(values_list, data_type)

...
try:
    ...
    class Serial:
        def __init__(self, *args, **kwargs):
            raise RuntimeError("pyserial not available - install pyserial package")
...
# Define SerialException separately to handle PyInstaller issues
try:
    import serial
    import serial.tools.list_ports
    if hasattr(serial, 'SerialException'):
        SerialException = serial.SerialException
    else:
        # Import directly from serialutil if needed
        import serial.serialutil
        SerialException = serial.serialutil.SerialException

    # For PyInstaller compatibility, ensure Serial is accessible
    Serial = serial.Serial
    SERIAL_AVAILABLE = True
except ImportError:
